...
adjacency_list_t Map::getDistMatrix(){
  return distMatrix;
}
...
void Map::loadMapCalcWeighted(const char *filename, bool outdoor, bool stairs, bool blacktop, bool paths){
  ifstream g(filename);
  if (nodes != 0)
  {
    delete[] nodes;
  ...
  for (int i = 0; i < size; ++i){  
	...
	b=nodes[vertex].getLocation();
	dist=a.distance(b);
	if (outdoor&&connectType==0){ dist*=2; }
	if (stairs&&connectType==2) { dist*=100;  }
	if (blacktop&&connectType==3) { dist*=20;  }
	if (paths&&connectType==4) {dist*=30; }
	distMatrix[i].push_back(Neighbor(vertex, dist));
    }
  }
}
...
void Map::drawConnections(const char *filename){
  ...
  for (int i = 0; i < size; ++i){  
	...
	if (i>vertex||true) rectangleDraw(one,two,1,connectType);
    }
  }
  size = sizeSave;
  nodes = nodesSave;
...
void Map::rectangleDraw(Point a, Point b, double height,int pathType){
  ...
  float x1,x2,x3,x4,y1,y2,y3,y4;
    ...
    glColor3f(1.,0.,0.);
    if (pathType==0) { glColor3f(1.,0.,0.); }
    if (pathType==1) { glColor3f(1.,1.,1.); }
    if (pathType==2) { glColor3f(0.,1.,0.); }
    if (pathType==3) { glColor3f(0.,0.,1.); }
    if (pathType==4) { glColor3f(.5,.5,.5); }
